      ...
      {products.map((product) => (
        <Link key={product._id} href={`/products/${product.slug}`} className="group text-sm">
          ...
          {/* {product.images && product.images.length > 0 ? ( */}
              ...
              />
            {/* ) : (
              // You can add a fallback image here if no images are available
              // Alternatively, you can handle this case based on your specific use case
              // For now, I'm just displaying some text as an example:
